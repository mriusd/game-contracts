...
//             db.RemoveItemFromDB(event.TokenId.int())
...
//             event.BlockNumber = receipt.BlockNumber
//             event.Coords = maps.Coordinate{X: event.CX.int(), Y: event.CY.int()}
...
//             if item.Name != "Gold" {
//                 _, _, err := fighter.GetBackpack().AddItem(tokenAtts, dropEvent.Qty.int(), someHash)
...
//             // get item attributes
//             fighterAttributes, _ := getFighterAttributes(itemId.int());
...
//     //tokenId := big.NewInt(0).Setint(item.TokenId)
...
//     var blockNumber = header.Number.Uint()
...
// func getTokenAttributes(itemId int) *items.TokenAttributes {
...
// func getFighterAttributes(TokenID int) (*fighters.FighterAttributes, error) {
...
// func getFighterMoney(fighter *fighters.Fighter) int {
...
//     roundedint, _ := rounded.int()
//     //log.Printf("[getFighterMoney] fighter:%v money: %v", id, roundedint)
//     return roundedint
// }
...
// // func getFighterStats(fighterID int) fighters.FighterStats {
...
// 	    	// get item attributes
// 	    	itemAttributes := getTokenAttributes(itemId.int());
